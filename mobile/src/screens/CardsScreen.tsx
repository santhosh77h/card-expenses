...
import CardSummaryRow from '../components/CardSummaryRow';
import SpendComparisonChart from '../components/charts/SpendComparisonChart';
import CategoryByCardChart from '../components/charts/CategoryByCardChart';
import SpendingSparkline from '../components/charts/SpendingSparkline';
import {
  getAvailableMonths,
  getCardSpendSummaries,
  getMonthlySpendByCard,
  getCategoryBreakdownByCard,
  getCardDetail,
  getActiveCurrencies,
  getCardMonthlyBills,
  getCardQuickStats,
...
export default function CardsScreen() {
  ...
  // Available months
  const availableMonths = useMemo(() => getAvailableMonths(monthlyUsage), [monthlyUsage]);
  const [selectedMonth, setSelectedMonth] = useState<string>(availableMonths[0] || '');

  // Keep selectedMonth in sync if data changes
  const effectiveMonth = availableMonths.includes(selectedMonth) ? selectedMonth : (availableMonths[0] || '');

  // Multi-currency: determine if we need a currency filter
  ...
  const cardSummaries = useMemo(
    () => effectiveMonth ? getCardSpendSummaries(cards, monthlyUsage, effectiveMonth, activeCurrencies.length > 1 ? effectiveCurrency : undefined) : [],
    [cards, monthlyUsage, effectiveMonth, effectiveCurrency, activeCurrencies.length],
  );

  const monthlySpend = useMemo(
    () => getMonthlySpendByCard(cards, monthlyUsage, 6, activeCurrencies.length > 1 ? effectiveCurrency : undefined),
    [cards, monthlyUsage, effectiveCurrency, activeCurrencies.length],
  );

  const categoryByCard = useMemo(
    () => effectiveMonth ? getCategoryBreakdownByCard(cards, statements, effectiveMonth, activeCurrencies.length > 1 ? effectiveCurrency : undefined) : [],
    [cards, statements, effectiveMonth, effectiveCurrency, activeCurrencies.length],
  );

  const cardDetail = useMemo(
    () => selectedCardId ? getCardDetail(selectedCardId, statements, monthlyUsage) : null,
    [selectedCardId, statements, monthlyUsage],
  );

  // Card colors for chart legend
  const cardColorMap = useMemo(
    () => cards.map((c) => ({ cardId: c.id, nickname: c.nickname, color: c.color })),
    [cards],
  );
          ...
          return (
            ...
            >
              <Feather
                name={icon}
                size={14}
                color={activeSegment === seg ? colors.accent : colors.textMuted}
...
function AnalyticsView({
  ...
  cardSummaries: ReturnType<typeof getCardSpendSummaries>;
  selectedCardId: string | null;
  onSelectCard: (id: string | null) => void;
  monthlySpend: ReturnType<typeof getMonthlySpendByCard>;
  categoryByCard: ReturnType<typeof getCategoryBreakdownByCard>;
  cardDetail: ReturnType<typeof getCardDetail> | null;
  selectedCard: CreditCard | null;
  cardColorMap: Array<{ cardId: string; nickname: string; color: string }>;
}) {
  if (!hasData) {
    return (
  ...
  return (
    <>
      ...
      {/* Currency filter (only if multiple currencies) */}
        ...
        <View style={styles.currencyRow}>
          {activeCurrencies.map((c) => (
            <TouchableOpacity
              ...
              style={[
                styles.currencyChip,
                effectiveCurrency === c && styles.currencyChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  effectiveCurrency === c && styles.currencyChipTextActive,
                ]}
              >
      ...
      {/* Per-card detail view OR overview charts */}
      {selectedCardId && cardDetail && selectedCard ? (
        <CardDetailView
          card={selectedCard}
          detail={cardDetail}
        />
...
// ---------------------------------------------------------------------------
// Overview Charts (all cards)
// ---------------------------------------------------------------------------

function OverviewChartsView({
  monthlySpend,
  categoryByCard,
  currency,
  cardColorMap,
  ...
  return (
    <View style={{ marginTop: spacing.lg }}>
      ...
      {monthlySpend.length === 0 && categoryByCard.length === 0 && (
        <EmptyState
          icon="pie-chart"
          title="No comparison data"
          subtitle="Upload statements for multiple months or cards to see comparison charts."
        />
      )}
    </View>
  );
...
function CardDetailView({
  ...
  card: CreditCard;
  detail: NonNullable<ReturnType<typeof getCardDetail>>;
}) {
  const currency = (card.currency || 'INR') as CurrencyCode;

  return (
    <View style={{ marginTop: spacing.lg }}>
      {/* Spending trend sparkline */}
      {detail.trendData.length >= 2 && (
        <View style={styles.detailSection}>
          <Text style={styles.detailTitle}>Spending Trend</Text>
          <View style={styles.sparklineContainer}>
            ...
            <View style={styles.sparklineLabels}>
              {detail.trendData.map((d, i) => (
                <Text key={d.month} style={styles.sparklineLabel}>{d.label}</Text>
              ))}
            </View>
          </View>
        </View>
      ...
      {detail.recentTransactions.length > 0 && (
        <View style={styles.detailSection}>
          <Text style={styles.detailTitle}>Recent Transactions</Text>
          {detail.recentTransactions.map((txn) => (
            <View key={txn.id} style={styles.txnRow}>
              <View style={[styles.txnDot, { backgroundColor: txn.category_color || colors.textMuted }]} />
              <View style={{ flex: 1 }}>
      ...
      {detail.trendData.length < 2 && detail.topCategories.length === 0 && (
        <EmptyState
          icon="credit-card"
          title="No data for this card"
          subtitle="Upload a statement for this card to see detailed analytics."
        />
      )}
    </View>
  );
...
// ---------------------------------------------------------------------------
// Manage View — Cards first, detail on selection, add form collapsed
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Statement Bar Chart — horizontally scrollable monthly usage bars
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Add Card Form — extracted for reuse
// ---------------------------------------------------------------------------
